...
| [DR-0086](DR-0086-tier-restrictiveness-declared.md) | Access-tier restrictiveness is declared, never derived from an ordering | security / architecture | **Proposed** | — |
...
of the "most restricted wins" rule its own rationale stated. The fix is
implemented and tested on the `gate-3-publication` branch; **the DR awaiting
founder decision is the rule it establishes**, that restrictiveness is
declared in both the schema and the export policy and never derived from any
ordering the database supplies.
