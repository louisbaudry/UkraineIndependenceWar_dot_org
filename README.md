...
| 2026-09-25 | **First civilian-harm source registered and collected on the archive server**, by the founder, step by step: `un-hrmmu-protection-of-civilians` (source `03411054-022a-41d6-998d-5803429169be`), August 2026 edition, run `c72bf2df-cb39-44d9-97b5-c72dc2067c53`: 3 acquired, 0 failed, 8 651 549 bytes, 0 documentary assertions; both PDFs' SHA-256 identical to the verification record, the landing page's changed because the UN edited it on 25 September. Found along the way: the live database lacks the `source_identity_unique` constraint the #62 fix added to the DDL (schema drift, #57). Monthly runs follow a person's update of `run_locators`, prompted on #82 | `docs/decision-records/DR-0110-un-hrmmu-registration.md` |
