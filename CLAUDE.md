...
| Item | State | Note |
|---|---|---|
| A1 — register the seven sanctions sources and make the first live collection | **in progress: 2 of 7 registered (2026-09-09), 2 more approved for registration but not yet executed (2026-09-12)** | `eu-consolidated-list` and `ofac-sdn` registered and collected on the archive server via `collector/run.py` (DR-0093). `uk-ofsi-consolidated` (fully) and `bis-entity-list` (Denied Persons List half only) are verified and the founder approved registering both (`DR-pending-second-source-registrations`); execution is on the archive server, per that record's *How to execute*, and has not happened yet. That record also flags a real gap: `register.py --commit`'s dependence-recording only links sources registered in the same call, so `uk-ofsi-consolidated`'s declared dependence on the already-registered `eu-consolidated-list` needs a direct SQL insert or a `register.py` fix, not a plain `--commit`. `eur-lex-sanctions`, `seco-sanctions`, `ua-nsdc-sanctions` remain unverified. Since 2026-09-12, `collector/run.py` also records a versioned software agent on preservation events (`DR-pending-collection-run-two-agents`) — exercised against two live sources in today's rehearsal, not yet on the archive server |
